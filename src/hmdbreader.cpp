      ...
      while(!in.atEnd()){
        ...
        if(line.isEmpty()){ // skip line
          ...
          if(line.contains("<?xml") == true){
            ...
            if(is_xml == true){
              QString sep = GetSeparator(line);
              //<version>3.5</version>
              line.replace("<", sep);
              line.replace(">", sep);
              line.remove("/");
              //;version;3.5;version;
              QStringList objs = line.split(sep, QString::SkipEmptyParts);
              if(objs.size() == 3){
                if(db->sdf_tab_contains(objs[0]) == false){
                  db->xml_tab.append(tab(objs[0]));
                  db->xml_tab.last().field_values.append(objs[1]);
                }
                else{
                  int indx = db->xml_field_index(objs[0]);
                  db->xml_tab[indx].field_values.append(objs[1]);
                }
              }
              else{
                qDebug() << objs;
              ...
              qDebug() << "Error while parsing xml file: " << filexml;
              break;
            }
          }
        }
